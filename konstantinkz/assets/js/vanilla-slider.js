  ...
  */
  Slider.getLimit = function (instance, pos) {
    ...
    if (direction === 'vertical' && pos === 'min') {
      return (((itemDimension + itemMargin) * (items.length - visibles + 0)) - itemMargin) * -1;

    }

    if (direction === 'horizontal' && pos === 'max') {
      return ((itemDimension + itemMargin) * (items.length - visibles + 0)) - itemMargin;
    }
  ...
  */
  ...
  Slider.fn.getNextPos = function (steps) {
    ...
    if (currentPos) {
      ...
      var currentWidth = document.getElementById("f").offsetWidth - 14;
      // alert(counter);
      if (counter < 2 ) {
        // alert(currentWidth);
        nextPos = currentPos + ((currentWidth) * steps);
        counter += 1
      } else {
        nextPos = currentPos;
      }
